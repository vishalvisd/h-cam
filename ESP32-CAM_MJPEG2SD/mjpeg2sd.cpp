...
int maxFrames = 1000; // maximum number of frames in video before auto close
...
static boolean processFrame() {
  ...
  static bool stoppedToRecapture = false;
  bool res = true;
...
//   isCapturing = forceRecord | captureMotion | pirVal;
  ...
  if (forceRecord || wasRecording || doRecording) {
    ...
    if (isCapturing && !wasCapturing) {
      ...
      openAvi();
      wasCapturing = true;
      stoppedToRecapture = false;
    }
    if (isCapturing && wasCapturing) {
      LOG_INF("visd frame count %u / %u frames", frameCnt, maxFrames);
      // capture is ongoing
    ...
    if (!isCapturing && wasCapturing) {
      ...
      if (!forceRecord && stoppedToRecapture) forceRecord = true;
    }
...
static void startSDtasks() {
  ...
  // visd change
  setFPS(4); // initial frames per second
